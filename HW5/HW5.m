...
for ii = 1:length(D)
    ...
    Z_hole(ii) = 1/(-1j* ((pi*a2^2)/(rho*c))*cot(k_fs4*0.85*L)); %Acoustic length of the hole simplified to be 0.85L as page 466 Rossing
...
Zl_g = 1j*k_gs4*rho*c*0.85*a2/S2;

Z_end_g = ZIN1(r_2, a2, D_max, Zl_g, k_gs4, rho, c);
Z_hole_g = 1/(-1j* ((pi*a2^2)/(rho*c))*cot(k_gs4*0.85*L));
Z_ending_g = 1./((1./Z_end_g) + (1./Z_hole_g));

Z_total_g = zeros(1, length(D_g));

for ii = 1:length(D_g)
    r1 = (L-D_g(ii))*tan(alpha);
